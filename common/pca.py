...
class PCAExample(object):
    ...
    def __init__(self, samples, target_dim):
        ...
        if target_dim != samples.shape[1]:
            self.__sub_origin = self.__sub_origin[:target_dim]
    ...
    def __estimate_subspace(self, samples):
        ...
        Returns: (Ergaenzen Sie die Dokumentation)
            sub_origin: Mittelwert jeder spalte in samples
            sub_var: Eigenwerte = Varianzen
            sub_vs: Eigenvektoren
        """
        # samples=si
        n_samples = float(samples.shape[0])
        # Mittelwert der Stichprobe (spaltenweise) oder u^
        samples_mean = np.sum(samples, axis=0) / n_samples
        sub_origin = samples_mean
        # Streuungsmatrix der Stichprobe
        X = samples - samples_mean  # (si-u^)
        # C^ sollte (si-u^)(si-u^)^T/N aber eigentlich  (si-u^)^T/N (si-u^)
        samples_cov = np.dot(X.T, X) / n_samples
        # Eigenwertanalyse der Streuungsmatrix
        sub_var, sub_vs = np.linalg.eig(samples_cov)  # Eigenwerten =sub_var und Eigenvektoren=sub_vs
        # Sortieren der Eigenvektoren (/ werte) nach Groesse des zugehoehrigen Eigenwerts
        sub_var_ind = np.argsort(sub_var)[::-1]  # a[::-1]    # all items in the array, reversed
        sub_var = sub_var[sub_var_ind]
        sub_vs = sub_vs[:, sub_var_ind]  # sortiern nach Spalten
